...
@property (nonatomic, strong) IBInspectable UIColor * _Nonnull invalidInputColor;
...
@property (nonatomic, copy) IBInspectable NSString * _Nullable cardNumberSeparator;
/// The duration of the view animation when switching from number input to detail.
@property (nonatomic) IBInspectable double viewAnimationDuration;
/// The text color for invalid input in a text field.
@property (nonatomic, strong) IBInspectable UIColor * _Nullable invalidInputColor;
/// Inset before the card type image view. Defaults to 1.0.
@property (nonatomic) IBInspectable CGFloat imageViewLeadingInset;
/// Inset after the card type image view. Defaults to 4.0.
@property (nonatomic) IBInspectable CGFloat imageViewTrailingInset;
@property (nonatomic) IBInspectable CGFloat accessoryButtonLeadingInset;
@property (nonatomic) IBInspectable CGFloat accessoryButtonTrailingInset;
@property (nonatomic, copy) NSString * _Nullable placeholder;
...
/// Use this class’ properties and functions to setup and
/// present the Fidel card linking feature.
SWIFT_CLASS_NAMED("Fidel")
@interface FLFidel : NSObject
/// All cards are linked with a program that you create on the Fidel platform.
/// Set the program identifier to this property to link cards to your program.
...
/// Sets your Fidel API Key. If you set a test API key,
/// then only test cards will be allowed to be linked.
/// If you set a production API key, then only real card numbers will be allowed.
...
/// The image to be displayed above the card input UI.
/// If it is not set, no image will be displayed
...
/// If this flag is set to <code>true</code>, it will open the card scanning feature,
/// right after presenting the Fidel card linking UI.
...
/// This <code>metaData</code> dicitonary will be associated with every card linked with your program.
...
/// These are the countries that the Fidel UI will allow the user to pick from.
/// If only one <code>Country</code> is set to this array, the country picker button will
/// not be available in the Fidel card linking UI.
/// The country will display the countries in the order that you specify in this array.
/// If you set an empty array of allowedCountries, the card linking UI will not be presented.
/// The first country in this array will be the default country.
/// The default value of this property is an array with all countries allowed by our service.
...
+ (void)setObjc_allowedCountries:(NSArray<NSNumber *> * _Nonnull)value;
...
@property (nonatomic, copy) IBInspectable NSString * _Nonnull cardNumberSeparator;
